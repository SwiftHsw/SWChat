...
#pragma mark 由于发送内容包含'符号会存储失败  统一转化为ATEN_HD   在这里统一做判断处理  碰到@"ATEN_HD" 转化为@"'"
    NSString *str = [self stringForColumnIndex:[self columnIndexForName:columnName]];
    if ([columnName isEqualToString:@"content"]) {
        if ([str containsString:@"ATEN_HD"]) {
            str = [str stringByReplacingOccurrencesOfString:@"ATEN_HD" withString:@"'"];
        }
    }
    return str;
}
