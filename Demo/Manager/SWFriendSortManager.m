...
@implementation SWFriendSortManager
...
+(NSMutableArray*)IndexArray:(NSArray*)stringArr{
    ...
    {
        NSString *pinyin = [((SWFriendSortManager *)object).pinYin substringToIndex:1];
//
        //不同
...
+(NSMutableArray*)LetterSortArray:(NSArray*)stringArr{
    ...
    for (NSString* object in tempArray) {
        ...
        NSString *pinyin = [((SWFriendSortManager *)object).pinYin substringToIndex:1];
        NSString *string = ((SWFriendSortManager *)object).string;
        //不同
        if(![tempString isEqualToString:pinyin])
        {
            //分组
...
+(NSMutableArray*)ReturnSortChineseArrar:(NSArray*)stringArr{
    ...
    {
        SWFriendSortManager *chineseString = [[SWFriendSortManager alloc]init];
        ...
        chineseString.string = [SWFriendSortManager RemoveSpecialCharacter:chineseString.string];
...
+(NSMutableArray*)IndexWithArray:(NSArray*)objectArray Key:(NSString *)key{
    ...
    {
        NSString *pinyin = [((SWFriendSortManager *)object).pinYin substringToIndex:1];
//        NSLog(@"pinyi = %@",((ATChineseSortTool *)object).pinYin);
        //不同
        if(![tempString isEqualToString:pinyin])
        {
            [A_Result addObject:pinyin];
...
+(NSMutableArray*)sortObjectArray:(NSArray*)objectArray Key:(NSString *)key{
    ...
    NSString *tempString;
    //拼音分组
    for (SWFriendSortManager* object in tempArray) {
        
        NSString *pinyin = [object.pinYin substringToIndex:1];
        
        id obj = object.object;
//        if ([obj isKindOfClass:[ATFriendInfoModel class]]) {
//            ATFriendInfoModel *ooo = (ATFriendInfoModel *)obj;
//            ooo.pingYin = [object.pinYin lowercaseString];
//            obj = ooo;
//        }else if ([obj isKindOfClass:[NSDictionary class]])
//        {
//            NSMutableDictionary *info = [[NSMutableDictionary alloc] initWithDictionary:obj];
//            [info setObject:[object.pinYin lowercaseString] forKey:@"pinYin"];
//            obj = info;
//        }
        //不同
        if(![tempString isEqualToString:pinyin])
...
+(NSMutableArray*)ReturnSortObjectArrar:(NSArray*)stringArr key:(NSString *)key{
    ...
    for(int i=0;i<[stringArr count];i++){
        ...
        if ([temp isKindOfClass:[SWFriendInfoModel class]]) {
            SWFriendInfoModel *info = (SWFriendInfoModel *)temp;
            if (!info.remark) {
                info.remark = @"";
                if (info.nickName) {
...
//    }
    ...
    NSString *sss = [pinyin stringByReplacingOccurrencesOfString:@" " withString:@""];
    result = [sss uppercaseString].mutableCopy;
    //判断第一个字符是否为字母
